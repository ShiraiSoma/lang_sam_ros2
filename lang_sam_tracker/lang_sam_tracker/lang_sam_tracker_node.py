...
class LangSamTrackerNode(Node):
    def __init__(self):
        ...
        self.detect_request_pub = self.create_publisher(CompressedImage, self.detect_request_topic, detect_qos)
    ...
    def timer_callback(self):
        ...
        # 生データ(bgr8)だと1枚あたり数百KB〜1MB近くになり、リモートとのTCP
        # トランスポート越しに送ると詰まりやすいため、JPEG圧縮して送る。
        ok, jpeg = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 85])
        if not ok:
            self.get_logger().warning('JPEGエンコードに失敗したため検出リクエストを送信しません')
            with self.state_lock:
                self.pending_stamp = None
                self.pending_frame = None
            return
